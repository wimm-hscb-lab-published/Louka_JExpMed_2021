...
# Subset HSCs only
    # Subset
    sample.info <- sample.info[which(sample.info$Sample %in% c("CB_HSC", "JMML_HSC")),]
    df.mat <- df.mat[, which(colnames(df.mat) %in% row.names(sample.info))]

    # Set factor levels
    sample.info$Sample <- factor(sample.info$Sample, levels=c("CB_HSC", "JMML_HSC"))
...
# Plot
    ...
    exp <- df.mat[gene.info.small$gene_id[1], , drop=FALSE]
